...
def custom_cross_validation(dataframe, classifier, permutation="random"):
    if permutation == "random":
        a = range(1, 25)
        permutation_t = np.random.permutation(a)
    else:
        permutation_t = permutation
    y_pred = []
    y_true = []
    for i in permutation_t:
        X_train, X_test, y_train, y_test = get_a_split(dataframe, i)
        y_true.append(y_test)
        classifier.fit(X_train, y_train)
        y_pred.append(classifier.predict(X_test))
    if permutation == "random":
        return permutation_t, y_true, y_pred
    else:
